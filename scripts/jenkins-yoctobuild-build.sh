...
if grep mender_servers <<<"$JOB_BASE_NAME"; then
    ...
    for docker in $($WORKSPACE/integration/extra/release_tool.py --list docker ); do (
        ...
        case "$docker" in
            deployments|deployments-enterprise|deviceauth|inventory|inventory-enterprise|tenantadm|useradm|useradm-enterprise|workflows|workflows-enterprise|workflows-worker|workflows-enterprise-worker|create-artifact-worker|auditlogs)
